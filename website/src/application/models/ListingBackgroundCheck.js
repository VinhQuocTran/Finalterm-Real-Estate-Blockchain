...
// Associates
ListingBackgroundCheck.belongsTo(BackgroundCheckService, { foreignKey: 'account_id' });
ListingBackgroundCheck.belongsTo(SubmitListingProperty, { foreignKey: 'account_id' });
...
ListingBackgroundCheck.addHook('beforeCreate', async (listingBackgroundCheck, options) => {
    ...
    const listingBackgroundId = `BCS_${counter.toString().padStart(4, '0')}`;
